...
import { QuizQuestion, fetchQuiz, partitionArray } from '@/app/lib/quizData';
...
export default function PracticeSimulationPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const partId = Number(params.part);
        const data = await fetchQuiz(quizId);
        
        // Partition into sets of 15
        const parts = partitionArray(data.questions, 15);
        if (partId >= 1 && partId <= parts.length) {
          setQuestions(parts[partId - 1]);
        } else {
          setQuestions(parts[0]);
        }
      } catch (e) {
        console.error(e);
      } finally {
  ...
  if (loading) {
    return (
      <main className="app-container">
        <div style={{ textAlign: 'center', marginTop: '4rem' }}>Loading practice...</div>
      </main>
    );
  }
  ...
  if (showResults) {
    return <ResultsSummary questions={questions} answers={answers} timeSpent={timeSpent} mode="practice" />;
  }
    ...
    <main className="app-container" style={{ maxWidth: '900px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <Link href="/practice" className="btn-outline">Restart</Link>
          <ThemeToggle />
        </div>
      </div>
